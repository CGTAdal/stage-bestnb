...
<?php 
	...
	if($_REQUEST['Update'] == 'Update'){
		...
		{	
			if ($_FILES["proof"]["name"]) 
			{
				...
				{ 
					...
					//@unlink($_POST['old_proof']);
					$proof = "../proofs/" . $date[0].$random_number."_".$_FILES["proof"]['name'];
				}	
			}else {
				$proof = trim($_POST['old_proof']);
			}		
		}
		...
		$data['stylename'] = trim($_POST['stylename']);
		$data['private'] = $_POST['private'];
		$data['frame'] = $_POST['frame'];
		$data['dome'] = $_POST['dome'];
		$data['proof'] = $proof;
		$where = "id = ".$_POST["id"];	
		modify_record("templates", $data, $where);
?>
<script language="javascript">
		//parent.parent.location.href = "admin_view_client_templates.php";
	//	window.close();
		</script>
<?php	
}
...
<div style="min-height: 300px;" class="portlet x12">
	<div class="portlet-header"><h4>Edit Template</h4></div>			
...
<form action="" method="post" name="add_template" id="add_template" enctype="multipart/form-data">	
	...
	<table  width="800" frame="box" border="0" align="center">
		...
		<tr>
			...
			<td>
				<input type="file" name="proof" value="" /><br />
				<?php if($row['proof'] !=''){?>
				<img src="<?php echo $row['proof']?>" style="max-width: 200px; max-height: 150px;">
				<?php } ?>
				<input type="hidden" value="<?php echo $row['proof']?>" name="old_proof"/>
			</td>
		</tr>
		<tr>
			<td><strong>Style name</strong></td>
			<td><input type="text" name="stylename" value="<?php echo $row['stylename']?>"></td>
		</tr>
		
		<tr>
			<td>Frame</td>
			<td><input <?php if($row['frame']==1){echo 'checked="checked"';}?> type="radio" name="frame" value="1"> None <input <?php if($row['frame']==2){echo 'checked="checked"';}?> type="radio" name="frame" value="2"> Silver <input <?php if($row['frame']==3){echo 'checked="checked"';}?> type="radio" name="frame" value="3"> Gold</td>
		</tr>
		<tr>
			<td>Dome</td>
			<td><input <?php if($row['dome']==1){echo 'checked="checked"';}?> type="radio" name="dome" value="1"> Yes <input <?php if($row['dome']==0){echo 'checked="checked"';}?> type="radio" name="dome" value="0"> No</td>
		</tr>
		...
		<tr>
			...
			<td>
				<select name="private">
					<option <?php if($row['private'] == 0){ echo 'selected';}?> value="0">Public</option>
					<option <?php if($row['private'] == 1){ echo 'selected';}?> value="1">Private</option>					
				</select>
			</td>
		</tr>
		<tr>
			<td align="right"><input style="font-size:12px;" class="btn btn-small" type="submit" name="Update" value="Update"></td>
