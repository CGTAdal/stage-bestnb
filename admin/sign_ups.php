...
if($_REQUEST['acp_id']){
	$data['sale_id'] = $_SESSION["loginid"];
	$data['follow_user_id'] = $_SESSION["loginid"];
	$data['is_sent'] = 1;
	$where = "id=".$_REQUEST['acp_id'];
...
<?php 
	...
	if($_SESSION['status']=='0'){
		$sql_count = "SELECT count(id) FROM customers WHERE is_sent = 0";													
	}else {
		$status = $_SESSION['status'];
		$sql_count = "SELECT count(id) FROM customers WHERE status ='{$status}'  AND  is_sent = 0";
	}
	//echo $sql_count;
	...
	if($_SESSION['status']=='0'){
		//die('aa');
		$sql  = "SELECT * FROM customers WHERE  is_sent = 0  ORDER BY ID DESC LIMIT {$start}, {$rec_limit}";											
	}else {
		$status_query = $_SESSION['status'];
		$sql  = "SELECT * FROM customers WHERE   is_sent = 0 AND status ='{$status_query }' ORDER BY ID DESC LIMIT {$start}, {$rec_limit}";											
	}
	//echo  $_SESSION['status'].'--'.$sql;
...
<div style="min-height: 300px;" class="portlet x12">
	<div class="portlet-header"><h4>Sign Ups</h4></div>			
		<div class="portlet-content" >
	<div style="float:right;font-size:12px" >
		<form action="sign_ups.php" name="filterform" method="post" style="font-size:13px">
		Filter by:
				<select id="filter" name="status" onchange="document.filterform.submit();">
					<option <?php if($_SESSION['status']==0){ echo 'selected';}?> value="0">All</option>
					<option <?php if($_SESSION['status']=='new'){ echo 'selected';}?> value="new">New</option>
					<option <?php if($_SESSION['status']=='Call Back'){ echo 'selected';}?> value="Call Back">Call Back</option>
					<option <?php if($_SESSION['status']=='Message'){ echo 'selected';}?> value="Message">Message</option>	
					<option <?php if($_SESSION['status']=='Archive'){ echo 'selected';}?> value="Archive">Archive</option>					
				</select> 				
		</form>
	</div>
	...
	<div style="margin-top: 5px;">
			<table border="0" class="customers" >
				<tr algin="left" bgcolor="#D8D7E3">
					...
					<td class="fieltable" width="200"><b>Sent to Sales</b></td>
				</tr>
				<tr>
				...
				<tr bgcolor="<?php echo $bgcolor;?>">
					...
					<td>
						<a href="sign_ups.php?acp_id=<?php echo $row['id'] ?>">Accept</a>
					</td>		
				</tr>	
				<tr bgcolor="<?php echo $bgcolor;?>">
	...
	<div style="margin-top: 10px;">
		<?php		
			doPages($rec_limit ,'/admin/sign_ups.php','',$rec_count);			
		?>
	</div>
	</div>
</div>
