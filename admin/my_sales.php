...
<?php 
	...
	if($_SESSION['status']=='0'){
		$sql_count = "SELECT count(id) FROM customers WHERE sale_id={$sale_id}";													
	}else {
		$status = $_SESSION['status'];
		$sql_count = "SELECT count(id) FROM customers WHERE status ='{$status}'  AND sale_id={$sale_id}";
	}
	//echo $sql_count;
	...
	if($_SESSION['status']=='0'){
		...
		$sql  = "SELECT * FROM customers WHERE sale_id= {$sale_id} ORDER BY ID DESC LIMIT {$start}, {$rec_limit}";											
	}else {
		$status_query = $_SESSION['status'];
		$sql  = "SELECT * FROM customers WHERE   sale_id= {$sale_id} AND status ='{$status_query }' ORDER BY ID DESC LIMIT {$start}, {$rec_limit}";											
	}
...
<div style="min-height: 300px;" class="portlet x12">
	<div class="portlet-header"><h4>My Sales</h4></div>			
		<div class="portlet-content" >
		
<div style="float:left;font-size:12px">
		<form action="my_sales.php" name="filterform" method="post" style="font-size:13px">
	...
	<div style="margin-top: 5px">
			<table border="0" class="customers">
				<tr algin="left" bgcolor="#D8D7E3">
					...
					<td class="fieltable"><b>Transfer</b>  </td>				
				</tr>
				<tr>
