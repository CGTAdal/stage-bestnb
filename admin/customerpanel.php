...
if ($_POST["process"])
{
	...
	$email_headers = email_header ("info@bestnamebadges.com", "Best Name Badges Print Order", "info@bestnamebadges.com", "info@bestnamebadges.com");
	ob_start();
	echo "Best Name Badges has received a new PRINT order, order number ".$id." from ".$customername.".<br>";
	
	$contents1 = ob_get_contents();
	ob_end_clean();
	mail("ryan@crucialclick.com", "Best Name Badges Print Order", $contents1, $email_headers);
...
if ($_POST["addname"])
{
	if ($_POST["badge"])
	{
		$qry = "SELECT inventory, finventory FROM customers WHERE id = '".$_SESSION["customerloginid"]."'";
    ...
    <div id="mainContentFull">
      ...
      <div id="logoBox" style="width: 450px;">
        ...
        <div style="text-align: left; line-height: 30px; margin-top:5px; margin-bottom: 5px; padding-left: 20px;">
            	<h4>You have <span class="quantityNumber"><?php echo $customer["inventory"]; ?></span> Pro Badges Available- <a href="purchase.php">Purchase More Inventory</a></h4>
                <h4>You have <span class="quantityNumber"><?php echo $customer["finventory"]; ?></span> Frames Available- <a href="purchase.php">Purchase More Inventory</a></h4> 
                <h4>Need a new badge design?- <a href="/order2.php">Purchase A New Style</a></h4> 
            </div>
         </div>
		 <?php } else { ?>
		 <div class="signUpField" >
        <div style="text-align: left; line-height: 30px; margin-top:5px; margin-bottom: 5px; padding-left: 20px; padding-right: 20px;">
            	<h4>You have never purchased a Pro Name Badge before. Please create your first badge by <a href="order2.php">clicking here.</a></h4>
        </div>
         </div>
		 <?php } ?>
       ...
       <!-- name -->
        <div class="signUpField" style="border-top-width: 1px; border-top-style: dashed; border-top-color: #CCC;">
            ...
            <div class="signUpFieldRight" style="width: 363px; font-size: 11px; height: 48px; line-height: 14px;">
                ...
                <a href="customerpanel.php?deleteid=<?php echo $badge["id"]; ?>&frame=<?php echo $badge["frame"]; ?>">Remove Name</a>
                </td></tr></table>
                </div>
            </div>
        </div>
