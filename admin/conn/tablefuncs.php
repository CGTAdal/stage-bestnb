...
function modify_record($table,$data,$where){
...
function delete_record_secondary($table, $rn, $id) {
		...
		if (mysql_query($query)) {
			...
			print "Failed to delete record subRecord";
			return "no";
		}
}
...
Paramaters:
	...
	$style is the order you want to sort in and can be either (0 or "ASC") for "ASC" or (1 or "DESC") for "DESC"
...
function get_records($table,$select,$where=0,$sortby=0,$style=0,$test=0,$limit=0,$hide_records=0,$key_type=MYSQL_BOTH,$force_array=1,$key=""){
	global $SC;
	
	if ($where) $where_string = "WHERE $where";
	if (!$select) $select = "*";
	if ($style==0 || $style=="ASC") $style_string = "ASC";
	if ($style==1 || $style=="DESC") $style_string = "DESC";
	if ($sortby) $sort_string = "ORDER BY $sortby $style_string";
...
/*********************************************************************************************************************************************
	...
	Purpose:
		This function adds a single record to the DB with an anchoring ID.
	
	Parameters:
		$table			table name (string)
		$data			array with field names as keys, and values rep. those field values
		$anchorfiled	name of anchor field
		$anchorid		ID of anchor field
	

*********************************************************************************************************************************************/


function add_anchoredrecord($table,$data,$anchorfield,$anchorid){
	...
	$query = "INSERT INTO $table ($field_string,$anchorfield) VALUES ($value_string,$anchorid)";
	//echo $query."<BR>";
	// if query is not successful, show error and return
	if (!mysql_query($query)) {
		echo "<b>Error:</b> ".mysql_error()."<br /><br /><b>Query was:</b> ".$query;
